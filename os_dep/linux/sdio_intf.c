...
#include <rtl8733b_hal.h>	/* rtl8733bs_set_hal_ops() */
...
#ifdef CONFIG_RTL8733B
	{SDIO_DEVICE(0x024c, 0xB733), .class = SDIO_CLASS_WLAN, .driver_data = RTL8733B}, /* SDIO+UART */
...
static void rtw_decide_chip_type_by_device_id(struct dvobj_priv *dvobj, const struct sdio_device_id  *pdid)
{
	...
	if (dvobj->chip_type == RTL8733B) {
		dvobj->HardwareType = HARDWARE_TYPE_RTL8733BS;
		RTW_INFO("CHIP TYPE: RTL8733B\n");
	}
#endif
}
...
u8 rtw_set_hal_ops(PADAPTER padapter)
{
	...
	if (rtw_get_chip_type(padapter) == RTL8733B)
		rtl8733bs_set_hal_ops(padapter);
#endif
